...
class Domain(Base):
    ...
    is_active=False — домен добавлен, но ещё не активирован (владелец не привязал DNS / оператор
    не подтвердил). На неактивном домене нельзя создавать ссылки (shorten → 400) и не выпускается
    TLS-сертификат (Caddy on-demand спрашивает /internal/tls-allow, который гейтит по is_active).
    """
