...
Веб-панель на `/admin` (доступна **только** на домене `ADMIN_HOST` — на остальных доменах
`/admin` отдаёт `404`; защищена логином). Вход — логин и пароль из env
(`ADMIN_USERNAME` / `ADMIN_PASSWORD`), сессия в подписанной куке (`SECRET_KEY`).
