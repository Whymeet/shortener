...
1. Ваш сервис шлёт `POST /shorten` с исходной ссылкой → получает `short_url`.
2. Вы раздаёте `short_url` (рассылка, кампания), при желании дописывая в неё свои параметры.
3. Пользователь кликает → наш сервис редиректит его (302) на исходную ссылку с влитыми
   параметрами и засчитывает клик.
...
| Поле | Требование |
|------|------------|
| `X-API-Key` | заголовок, обязателен; ваш ключ |
| `full_link` | поле в JSON-теле; исходная ссылка на `http://`/`https://`, длина ≤ 4096 |

> ⚠️ **Домен короткой ссылки = адрес, на который вы шлёте запрос.**
> Отдельного поля `domain` в теле **НЕТ** — тело содержит ровно одно поле `full_link`.
> Куда постучались, на том домене и создастся ссылка: запрос на `https://<КОРОТКИЙ_ДОМЕН>/shorten`
> → `short_url` будет на `<КОРОТКИЙ_ДОМЕН>`. Заголовок `Host` ваш HTTP-клиент проставляет
> автоматически из URL — вручную ничего добавлять не нужно.
